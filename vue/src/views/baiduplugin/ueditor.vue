<template>
  <div class="hello">
    <div id="preview" @click="showData" class="preview" v-html="msg" v-show="show"></div>
    <!-- 你不需要担心一个页面中过多vue-ueditor-wrap组件会导致混乱,或者重复引用JS(打开控制台瞄一眼),每个组件都保证拥有一个独立的UEditor实例,你可以尝试把数字改成99(根据自己电脑自行斟酌),但依然稳定可用 -->
    <vue-ueditor-wrap ref="ueditor" v-model="msg" :destroy="false" :config="config" @ready="ready" v-for="item in 2" :key="item" @beforeInit="addCustomUI" style="z-index: 1999"></vue-ueditor-wrap>
    <vue-ueditor-wrap ref="ueditor1" v-model="msg1" :destroy="false" :config="config" @ready="ready" @beforeInit="addCustomUI" style="z-index: 1999"></vue-ueditor-wrap>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      show: true,
      // 3、v-model绑定数据
      msg: '配置后台文件上传spring.servlet.multipart.enabled=false',
      msg1: '',
  ...
  beforeDestroy () {
    localStorage.setItem('uData', this.msg)
  },
  methods: {
